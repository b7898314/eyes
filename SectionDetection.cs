...
namespace bearing
{
    class SectionDetection
    {
        public class myReverserClass : IComparer
        {
            ...
            {
                Contour<Point> a = x as Contour<Point>;
                Contour<Point> b = y as Contour<Point>;
                double aSize = a.BoundingRectangle.Width * a.BoundingRectangle.Height;
                double bSize = b.BoundingRectangle.Width * b.BoundingRectangle.Height;
                if (aSize > bSize)
                {
                    return -1;
                }
        ...
        public void GeometryDetection(Image<Bgr, Byte> src, Image<Bgr, Byte> dest, double upperCircleRadius, double lowerCircleRadius, out CircleF[] closeCircle, out double[] STD)
        {
            ...
            CvInvoke.cvThreshold(grayImage, otsuImage, 128, 255, THRESH.CV_THRESH_OTSU);
            //contour
            Contour<Point> contours = otsuImage.FindContours(CHAIN_APPROX_METHOD.CV_CHAIN_APPROX_NONE, RETR_TYPE.CV_RETR_LIST);
            ArrayList contoursArrayList = new ArrayList();
            for (; contours != null; contours = contours.HNext)
            {
                //dest.Draw(contours.ApproxPoly(1), new Bgr(Color.Blue), 1);
                contoursArrayList.Add(contours);
            }
            ...
            STD = new double[2];
            //最小平方圓
            closeCircle[0] = GetCloseCircle(((Contour<Point>)contoursArrayList[0]).ToArray());
            ...
            {
                Point[] points = ((Contour<Point>)contoursArrayList[0]).ToArray();
            ...
            {
                closeCircle[1] = GetCloseCircle(((Contour<Point>)contoursArrayList[1]).ToArray());
                closeCircle[2] = GetMaxMinCircle(((Contour<Point>)contoursArrayList[0]).ToArray(), closeCircle[0], 0);
                closeCircle[3] = GetMaxMinCircle(((Contour<Point>)contoursArrayList[0]).ToArray(), closeCircle[0], 1);
                closeCircle[4] = GetMaxMinCircle(((Contour<Point>)contoursArrayList[1]).ToArray(), closeCircle[1], 0);
                closeCircle[5] = GetMaxMinCircle(((Contour<Point>)contoursArrayList[1]).ToArray(), closeCircle[1], 1);
                //DrewCloseCircle(ref dest, closeCircle[0], Color.Yellow);
                //DrewCloseCircle(ref dest, closeCircle[1], Color.Yellow, 1);
                //DrewCloseCircle(ref dest, closeCircle[2], Color.Red, 2);
                //DrewCloseCircle(ref dest, closeCircle[3], Color.Green, 3);
                //DrewCloseCircle(ref dest, closeCircle[4], Color.Red, 4);
                //DrewCloseCircle(ref dest, closeCircle[5], Color.Green, 5);
                STD[0] = CalculateSTD(((Contour<Point>)contoursArrayList[0]).ToArray(), closeCircle[0]);
                STD[1] = CalculateSTD(((Contour<Point>)contoursArrayList[1]).ToArray(), closeCircle[1]);
            }
        }
        //最小平方圓
